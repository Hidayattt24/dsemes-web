...
export function QuizFormFeature({ quizId }: QuizFormFeatureProps) {
  ...
  // Modal confirmation for deleting a Category
  const [categoryToDeleteIndex, setCategoryToDeleteIndex] = useState<number | null>(null);

  // Accordion state for Collapsible Questions (key: `${catIdx}_${qIdx}`)
  const [collapsedQuestions, setCollapsedQuestions] = useState<Record<string, boolean>>({});

  const toggleQuestionCollapse = (catIdx: number, qIdx: number) => {
  ...
  return (
    <div className="max-w-[1400px] mx-auto w-full font-[family-name:var(--font-poppins)]">
      {/* Page Header & Actions */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 border-b border-[#E2E8F0] pb-4">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-[#718096] mt-1 min-h-[20px] flex items-center">
            {isPreTest
              ? "Kuesioner Pre-Test untuk mengukur pengetahuan awal pasien sebelum edukasi (tanpa passing score & tanpa materi)"
              : "Kuesioner Post-Test untuk mengukur pemahaman setelah membaca materi edukasi tertentu"}
          </p>
        </div>
      ...
      <form className="space-y-8">
        {/* Section 1: Dynamic Form Type & Basic Info */}
              ...
              >
                ...
                <div className="pt-3 border-t border-teal-100/80 grid grid-cols-1 gap-1.5 text-[11px] font-bold text-[#4A5568]">
                  <div className="flex items-center gap-1.5 text-[#00695C]">
                    ...
                    <span>Tanpa Passing Score & Tanpa Nilai Kelulusan</span>
                  </div>
                  <div className="flex items-center gap-1.5 text-[#00695C]">
                    <span className="material-symbols-outlined text-sm">link_off</span>
                    <span>Bebas (Tidak Terikat 1 Materi Edukasi)</span>
                  </div>
                  <div className="flex items-center gap-1.5 text-[#00695C]">
                    <span className="material-symbols-outlined text-sm">stars</span>
                    <span>Hanya 1 Pre-Test Aktif di dalam Sistem</span>
                  </div>
                </div>
              </div>
              ...
              >
                ...
                <div className="pt-3 border-t border-teal-100/80 grid grid-cols-1 gap-1.5 text-[11px] font-bold text-[#4A5568]">
                  <div className="flex items-center gap-1.5 text-[#00695C]">
                    ...
                    <span>Wajib Memiliki Passing Score (Nilai Kelulusan)</span>
                  </div>
                  <div className="flex items-center gap-1.5 text-[#00695C]">
                  ...
                  <div className="flex items-center gap-1.5 text-[#00695C]">
                    ...
                    <span>Memiliki Tingkat Kesulitan (Mudah / Sedang / Sulit)</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* Section 2: Question Categories Builder */}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-[#E2E8F0] shadow-sm">
            <div>
              <h3 className="text-base font-bold text-[#1A202C] flex items-center gap-2">
                ...
                <span>Kategori Pembelajaran & Soal</span>
              </h3>
            ...
            {fields.categories.map((category, catIdx) => (
              ...
              >
                ...
                <div className="space-y-6 pt-2">
                      ...
                      <div className="space-y-3 pt-2">
                        <div className="flex items-center justify-between">
                          <label className="block text-xs font-bold text-[#4A5568]">
                            Pilihan Jawaban (Tandai tombol radio pada jawaban yang benar) <span className="text-red-500">*</span>
                          </label>
                          <button
                          ...
                          {question.choices.map((choice, choiceIdx) => (
                            ...
                            >
                              ...
                              <input
                                ...
                                placeholder={`Ketik Pilihan Jawaban ${String.fromCharCode(65 + choiceIdx)}...`}
                              />
                              {choice.isCorrect && (
                  ...
                  {/* Add Question Button for this Category */}
                  <div className="pt-2">
                    <button
